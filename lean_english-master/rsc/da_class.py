...
class LopKyNang(KhungBaiHoc):
    ...
    def __init__(self, level, so_bai, ky_nang, language='en-US', **kwargs):
        ...
        # Khởi tạo Microphone chuẩn xác trong __init__
        try:
            with sr.Microphone() as source:
                self.recognizer.adjust_for_ambient_noise(source, duration=0.5)
            self.has_mic = True
        except Exception as e:
            print(f"\n[Cảnh báo] Micro lỗi: {e}")
            self.has_mic = False

    # SỬA LẠI: Đưa hàm này ra ngoài, ngang hàng với __init__ (lùi vào 4 dấu cách)
    def thu_am_tra_text(self, language='en-US', timeout=60, phrase_time_limit=60):
        """Ghi âm -> chuyển text"""
        if not self.has_mic:
            return ""
        ...
        try:
            ...
            print(f" Lỗi mic hoặc không nhận diện được: {e}")
            return ""

    # THÊM MỚI: Định nghĩa hàm này ở lớp cha để LopNghe, LopDoc, LopViet, LopNoi đều gọi được
    def _listen_once(self, timeout=60, phrase_time_limit=60):
        """Hàm đồng bộ cấu hình gọi từ giao diện GUI"""
        return self.thu_am_tra_text(language=self.language, timeout=timeout, phrase_time_limit=phrase_time_limit)
    ...
    def kiem_tra_dap_an(self, user_input, dap_an_dung):
        """
        So sánh đáp án
        """
        ...
        if isinstance(dap_an_dung, list):
            return user_input in [d.lower() for d in dap_an_dung if d]

        return user_input == dap_an_dung.lower()
...
class LopNghe(LopKyNang):
    ...
    def muc1(self):
        ...
        if not user_input:
            user_input = self.thu_am_tra_text()
...
class LopNoi(LopKyNang):
    ...
    def thuc_hien_muc(self):
        ...
        for lan in range(3):
            ...
            if self.kiem_tra_dap_an(user_answer, dap_an_dung):
                print("✓ Chính xác!")
                return True
            else:
