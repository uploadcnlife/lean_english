...
class MainApp(QMainWindow, Ui_khungchung):
    def __init__(self):
        ...
        self.correct_answer_muc1 = ""
        self.correct_answer_muc2 = ""
        ...
        self.pushButton_12.clicked.connect(self.record_speaking_answer)
        self.pushButton_11.clicked.connect(self.record_speaking_answer)
        self.pushButton_13.clicked.connect(self.record_speaking_answer)
        self.pushButton_14.clicked.connect(self.record_speaking_answer)
        self.pushButton_15.clicked.connect(self.record_speaking_answer)
        self.pushButton_16.clicked.connect(self.record_speaking_answer)
    ...
    def record_speaking_answer(self):
        """Ghi âm cho các kỹ năng Nghe, Nói, Đọc, Viết dựa theo nút bấm được nhấn"""
        if not self.skill_obj:
            return

        # Xác định nút bấm nào phát ra sự kiện
        nut_bam = self.sender()
        if not nut_bam:
            return

        # Ánh xạ kết nối logic tương ứng: Nút bấm -> Đáp án đúng -> Ô thông báo giao diện
        if nut_bam == self.pushButton_10:
            correct = self.correct_answer_muc1
            msg_box = self.tbao_noi_2
        elif nut_bam == self.pushButton_12:
            correct = self.correct_answer_muc1
            msg_box = self.dan_nghe_1
        elif nut_bam == self.pushButton_11:
            correct = self.correct_answer_muc2
            msg_box = self.tbao_nghe_2
        elif nut_bam == self.pushButton_14:
            correct = self.correct_answer_muc1
            msg_box = self.tbao_doc_1
        elif nut_bam == self.pushButton_13:
            correct = self.correct_answer_muc2
            msg_box = self.tbao_doc_2
        elif nut_bam == self.pushButton_16:
            correct = self.correct_answer_muc1
            msg_box = self.tbao_viet_1
        elif nut_bam == self.pushButton_15:
            correct = self.correct_answer_muc2
            msg_box = self.tbao_viet_2
        else:
            return

        # Thông báo trạng thái đang lắng nghe trực tiếp lên ô kết quả của bài học
        msg_box.setPlainText(" Đang nghe... Hãy nói ngay!")
        QApplication.processEvents()  # Cập nhật giao diện lập tức để người dùng thấy chữ "Đang nghe"

        # Bọc try-except để lấy kết quả thu âm thoại
        try:
            # Tăng thời gian phrase_time_limit lên 7 giây để bạn kịp đọc câu dài
            user_ans = self.skill_obj._listen_once(timeout=5, phrase_time_limit=140)
        except Exception as e:
            msg_box.setPlainText(f" Lỗi thiết bị mic: {str(e)}")
            return

        # Nếu không nhận diện được (chuỗi rỗng), gán chuỗi rỗng để so sánh và báo sai
        if not user_ans:
            user_ans = ""

        # Tiến hành so sánh chuỗi đáp án chuẩn hóa
        if user_ans.strip().lower() == correct.strip().lower():
            msg_box.setPlainText(f" Nhận diện: {user_ans}\n✓ Chính xác!")
        else:
            if user_ans == "":
                msg_box.setPlainText(f" Không nghe được giọng nói.\n✗ Sai. Đáp án đúng: {correct}")
            else:
                msg_box.setPlainText(f" Nhận diện: {user_ans}\n✗ Sai. Đáp án đúng: {correct}")
    # ------------------ XỬ LÝ TRẢ LỜI ------------------
    ...
    def show_login_ui(self):
        try:
            # 1. Định vị chính xác thư mục 'lean_english-master' từ file đang chạy
            current_path = Path(__file__).resolve()
            project_source_dir = None

            # Quét ngược lên trên để tìm thư mục nào chứa đồng thời cả 'GUI' và 'rsc' như trong ảnh
            for parent in current_path.parents:
                if (parent / "GUI").exists() and (parent / "rsc").exists():
                    project_source_dir = parent
                    break

            # Nếu không quét ra (trường hợp khẩn cấp), tự lùi 1 cấp từ vị trí main.py hiện tại
            if not project_source_dir:
                project_source_dir = current_path.parent

            # 2. Tạo đường dẫn tuyệt đối đến file MOI.py
            ...
            if moi_path_obj.exists():
                ...
                # 3. Cấu hình PYTHONPATH trỏ thẳng vào 'lean_english-master' để nhận diện thư mục 'rsc'
                env = os.environ.copy()
                ...
                # 4. Chạy file qua CMD để nếu có lỗi phát sinh trong MOI.py (ví dụ thiếu thư viện khác) nó sẽ hiện ra màn hình đen
                subprocess.Popen(
